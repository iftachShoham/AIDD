...
from core.utils.catsample import sample_categorical



def _as_col(x):
    return x if x.ndim == 2 and x.shape[1] == 1 else x.reshape(-1, 1)
...
class Absorbing(Graph):
    def __init__(self, dim, p_base=0.25, alpha=0.0):
        super().__init__()
        self._dim = dim
        self.p_base = p_base
        self.alpha = alpha
    ...
    def sample_transition(self, i, sigma):
        ...
        move_indices = torch.zeros_like(i, dtype=torch.bool)

        for b in range(B):
            budget = int((move_chance[b, 0] * L).item())
            p_sigma = self.p_base / (1 + self.alpha * sigma[b].item())
            flipped = 0

            while flipped < budget:
                u = torch.rand(1).item()
                span_len = int(torch.clamp(
                    torch.floor(torch.log(torch.tensor(u)) / torch.log(torch.tensor(1 - p_sigma))) + 1,
                    max=30
                ))
                start = torch.randint(0, L - span_len + 1, (1,)).item()
                move_indices[b, start:start + span_len] = True
                flipped = int(move_indices[b].sum().item())
